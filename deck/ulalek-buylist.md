# Ulalek Bracket-3 Buylist — 2026-08-18 prices (Cardmarket EUR via Scryfall)

Prices are Cardmarket EUR (nonfoil) via the Scryfall API, snapshot 2026-08-18, default printing; where
the default printing had no EUR price, the cheapest EUR printing was used. Card counts follow
`deck/ulalek-bracket3.txt`, which is the canonical source for names. Only the physical collection
(`archidekt-collection-export-2026-08-17.csv`) counts as owned — Zhulodok buylist cards are priced
as real purchases here. Proxying freely is fine per the owner, so expensive cards are kept in the
list and proxied rather than replaced with budget substitutes.

## ⚠ Cards to double-check before acquiring (approaching/over 100€)

No card in the list reaches 100€ at this snapshot, but two are close (and cross $100 in USD):

| Card | EUR | USD |
|---|---|---|
| Ancient Tomb | 97.84 | 134.58 |
| Mana Vault | 73.37 | 118.26 |

Both are on the proxy list below.

## Bracket-3 changes vs `eldrazi-incursion-20260818-163950.txt`

Bracket 3 forbids early-game 2-card infinite combos; late-game combos are allowed. Only the combo
noted in the decklist was removed:

| Cut | Why |
|---|---|
| Food Chain | Food Chain + Eternal Scourge is an early-game infinite (noted in decklist) |
| Eternal Scourge | other half of the combo |
| Glaring Fleshraker | noted alongside them — its Ulalek + Echoes line is the fast kill |

Added in their place: Emrakul, the World Anew; Not of This World; Abstruse Archaic.

Kept deliberately (late-game lines, verified as the only combos Commander Spellbook finds in the
final list): Ulalek + Echoes of Eternity + Kozilek's Unsealing, and Basalt Monolith + Forsaken
Monument. Game Changers sit at exactly 3 — Ancient Tomb, Mana Vault, Crop Rotation — the
bracket-3 cap.

## 2026-08-19 speed/resilience pass

Five swaps to fix the slow "7 mana → doubled-Eldrazi turn" conversion and the T5–8 danger window
(see decision log in `deck/ulalek-bracket3.txt`). Out: Shamanic Revelation, Selective Obliteration,
Endbringer, Sire of Stagnation, Benthic Anomaly (−2.97€ from the Buy list). In: Mind Stone,
Fellwar Stone, Worn Powerstone, Elder Deep-Fiend, Lightning Greaves (+4.40€, priced 2026-08-19).
The fatty slot was first Metalwork Colossus, revised same day after owner review — Elder Deep-Fiend
is an actual Eldrazi (Ulalek copy) with a flash defensive mode, and the sim showed Colossus wasn't
carrying the speed gain. Combos re-verified after both revisions: still only the two allowed
late-game lines above; no new Game Changers (none of the adds is on the list).

## 2026-08-19 interaction pass

Two swaps from the same-day reassessment (interaction in the T3–6 window was a coin flip and soft
in quality; see decision log in `deck/ulalek-bracket3.txt`). Out: Null Elemental Blast (multicolored-
only, frequently dead), Eldrazi Mimic (−1.21€). In: Beast Within, Imprisoned in the Moon (+0.79€,
priced 2026-08-19). No Game Changer or combo-policy implications.

## Owned (0€, from collection)
| Card | Note |
|---|---|
| Eldrazi Temple | from collection |
| Eye of Ugin | from collection (44.77€ card — big save) |
| Sol Ring | from collection |
| Eldrazi Conscription | from collection |

## Proxy (would cost ≈472.52€ — print these; ≥25€ each)
| Card | EUR | Note |
|---|---|---|
| Ancient Tomb | 97.84 | ⚠ double-check; Game Changer, core fast mana |
| Mana Vault | 73.37 | ⚠ double-check ($118 USD); Game Changer |
| The Banyan Tree | 49.31 | this is the Avatar: TLA printing of The Great Henge |
| Mana Drain | 46.98 | |
| Zhulodok, Void Gorger | 45.78 | foil-only precon exclusive, no nonfoil printing exists |
| Cavern of Souls | 40.66 | makes every Eldrazi uncounterable |
| Ugin, Eye of the Storms | 37.54 | |
| Prismatic Vista | 28.74 | |
| Scalding Tarn | 26.64 | |
| Misty Rainforest | 25.66 | |

## Buy (total: 373.48€, 86 cards)
| Card | EUR |
|---|---|
| Wasteland | 23.39 |
| Mana Confluence | 23.02 |
| Summon: Bahamut | 21.95 |
| Three Tree City | 19.30 |
| Shadow in the Warp | 18.82 |
| Ulamog, the Ceaseless Hunger | 18.05 |
| Wooded Foothills | 16.80 |
| Ugin's Labyrinth | 15.58 |
| Urza's Incubator | 15.32 |
| Flayer of Loyalties | 15.11 |
| Ulamog, the Defiler | 15.05 |
| Ketria Triome | 13.64 |
| Ugin, the Spirit Dragon | 11.01 |
| Skittering Cicada | 10.81 |
| Yavimaya, Cradle of Growth | 10.45 |
| Breeding Pool | 9.33 |
| Steam Vents | 8.72 |
| City of Brass | 8.15 |
| Kozilek's Command | 7.21 |
| Stomping Ground | 6.95 |
| Emrakul, the World Anew | 4.99 |
| Kozilek, the Great Distortion | 4.63 |
| Three Visits | 4.55 |
| All Is Dust | 4.18 |
| Stubborn Denial | 3.59 |
| Crop Rotation | 3.47 |
| Conduit of Ruin | 3.42 |
| Kozilek, the Broken Reality | 3.41 |
| Thought-Knot Seer | 3.41 |
| Eldritch Immunity | 3.36 |
| Forsaken Monument | 3.34 |
| Lightning Greaves | 3.02 |
| Not of This World | 2.87 |
| Basalt Monolith | 2.85 |
| Desecrate Reality | 2.61 |
| Nature's Lore | 2.46 |
| Nulldrifter | 1.99 |
| Sowing Mycospawn | 1.74 |
| Sanctum of Ugin | 1.36 |
| Abstruse Archaic | 1.29 |
| Ugin's Binding | 1.24 |
| Ugin, the Ineffable | 1.20 |
| Cascading Cataracts | 1.11 |
| Reality Smasher | 1.08 |
| Hideous Taskmaster | 1.05 |
| Kozilek's Return | 0.94 |
| Fellwar Stone | 0.93 |
| Karplusan Forest | 0.92 |
| From Beyond | 0.86 |
| Petrified Hamlet | 0.80 |
| Echoes of Eternity | 0.75 |
| Eldrazi Displacer | 0.75 |
| Ultima, Origin of Oblivion | 0.69 |
| Beast Within | 0.64 |
| Arcane Signet | 0.53 |
| Warping Wail | 0.50 |
| Shivan Reef | 0.48 |
| Bonders' Enclave | 0.46 |
| Eldrazi Linebreaker | 0.46 |
| Yavimaya Coast | 0.44 |
| Chittering Dispatcher | 0.43 |
| Tomb of the Spirit Dragon | 0.41 |
| Talisman of Impulse | 0.39 |
| Herald of Kozilek | 0.37 |
| Blast Zone | 0.36 |
| Ulalek, Fused Atrocity (commander) | 0.35 |
| Azlask, the Swelling Scourge | 0.34 |
| Awakening Zone | 0.33 |
| Talisman of Curiosity | 0.32 |
| Thief of Existence | 0.32 |
| Command Tower | 0.28 |
| Snow-Covered Wastes | 0.28 |
| It That Heralds the End | 0.27 |
| Kozilek's Unsealing | 0.26 |
| Exotic Orchard | 0.21 |
| World Breaker | 0.21 |
| Sylvan Scrying | 0.18 |
| Mind Stone | 0.18 |
| Worn Powerstone | 0.17 |
| Imprisoned in the Moon | 0.15 |
| Path of Ancestry | 0.14 |
| Elder Deep-Fiend | 0.10 |
| Forest | 0.10 |
| Island | 0.10 |
| Mountain | 0.10 |
| Path of Annihilation | 0.10 |

## Cost summary
- Cash outlay if all Buy cards are purchased: **373.48€** (86 cards; the seven 2026-08-19 adds are
  priced at that day's snapshot, the rest at 2026-08-18)
- Avoided by proxying (10 cards ≥25€): 472.52€
- The 25€ proxy threshold is a default — since proxying freely is on the table, any Buy card can
  move to the proxy pile to cut the outlay further (the top 12 Buy cards alone are ≈213€).
